...
class EnergyMethod:
    def __init__(self, k_dens, k_func, dim_hid, no_layers, act_func, std_dev_rff, optimizer, lr, beta, mb_factor, epochs, directory):
        ...
        if method == 'DEM':
            ...
            self.dRBF = self.model.getDerivative()
            self.outRBF = self.model.rbf_layer(X)
    ...
    def getU(self, X):
        ...
        U = torch.stack((Z[:, 0] * X[:, 0], Z[:, 1] * X[:, 0]), dim=1)
        return Z, U
    ...
    def getLoss(self, losses):
        ...
        if method == 'DEM':
            duxdX = torch.autograd.grad(U[:, 0], X, torch.ones(X.size(0)), create_graph=True)[0]
            duydX = torch.autograd.grad(U[:, 1], X, torch.ones(X.size(0)), create_graph=True)[0]
        elif method == 'SEM':
            # derivatives of model output w.r.t. rbf layer
            duxdRBF = torch.autograd.grad(Z[:, 0], self.outRBF, torch.ones(X.size(0)), create_graph=True)[0].unsqueeze(1)
            duydRBF = torch.autograd.grad(Z[:, 1], self.outRBF, torch.ones(X.size(0)), create_graph=True)[0].unsqueeze(1)
            # chain rule - derivatives of model output w.r.t. input
            duxdX = torch.bmm(duxdRBF, self.dRBF).squeeze()
            duydX = torch.bmm(duydRBF, self.dRBF).squeeze()
            # product rule - derivatives of modified model output (with BCs) w.r.t. input
            duxdX[:, 0] *= X[:, 0]
            duxdX[:, 1] *= X[:, 0]
            duxdX[:, 0] += Z[:, 0]
            duydX[:, 0] *= X[:, 0]
        ...
        if len(losses) < 200:
            Psi1, S1 = self.material1.getInnerEnergy('Hooke', duxdX, duydX)[:2]
        else:
            Psi1, S1 = self.material1.getInnerEnergy(mat_mod1, duxdX, duydX)[:2]
        ...
        if multiphase:
            ...
            if len(losses) < 200:
                Psi2, S2 = self.material2.getInnerEnergy('Hooke', duxdX, duydX)[:2]
            else:
                Psi2, S2 = self.material2.getInnerEnergy(mat_mod2, duxdX, duydX)[:2]
...
if __name__ == '__main__':
    ...
    Z, U = energymethod.getU(X)
    if method == 'DEM':
        duxdX = torch.autograd.grad(U[:, 0], X, torch.ones(X.size(0)), create_graph=True)[0]
        duydX = torch.autograd.grad(U[:, 1], X, torch.ones(X.size(0)), create_graph=True)[0]
    elif method == 'SEM':
        # derivatives of model output w.r.t. rbf layer
        duxdRBF = torch.autograd.grad(Z[:, 0], energymethod.outRBF, torch.ones(X.size(0)), create_graph=True)[0].unsqueeze(1)
        duydRBF = torch.autograd.grad(Z[:, 1], energymethod.outRBF, torch.ones(X.size(0)), create_graph=True)[0].unsqueeze(1)
        # chain rule - derivatives of model output w.r.t. input
        duxdX = torch.bmm(duxdRBF, energymethod.dRBF).squeeze()
        duydX = torch.bmm(duydRBF, energymethod.dRBF).squeeze()
        # product rule - derivatives of modified model output (with BCs) w.r.t. input
        duxdX[:, 0] *= X[:, 0]
        duxdX[:, 1] *= X[:, 0]
        duxdX[:, 0] += Z[:, 0]
        ...
        duydX[:, 1] *= X[:, 0]
        duydX[:, 0] += Z[:, 1]
    S1 = energymethod.material1.getInnerEnergy(mat_mod1, duxdX, duydX)[-1].cpu().detach()
    S = S1
    if multiphase:
        S2 = energymethod.material2.getInnerEnergy(mat_mod2, duxdX, duydX)[-1].cpu().detach()
